...
export async function GET() {
  ...
  const rows = await db.select().from(partnerEnquiries).orderBy(desc(partnerEnquiries.createdAt));
  return jsonOk(rows);
}
...
export async function PUT(request: NextRequest) {
  ...
  if (!id || !status) return jsonError("Partner enquiry id and status are required");
  const [row] = await db
    .update(partnerEnquiries)
    .set({ status })
    .where(eq(partnerEnquiries.id, id))
    .returning();
  if (!row) return jsonError("Partner enquiry not found", 404);
  return jsonOk(row);
}
...
export async function DELETE(request: NextRequest) {
  ...
  if (!id) return jsonError("Partner enquiry id is required");
  const [row] = await db.delete(partnerEnquiries).where(eq(partnerEnquiries.id, id)).returning();
  if (!row) return jsonError("Partner enquiry not found", 404);
